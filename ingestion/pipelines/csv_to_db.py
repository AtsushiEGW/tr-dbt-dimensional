    ...
    try:
        ...
        for f in processed_files:
            ...
            for chunk in pd.read_csv(
                f,
                header=None,                 # ← names を使うのでヘッダは無しとして読む
                names=norm_cols,             # 正規化済みヘッダで固定
                usecols=range(len(norm_cols)),  # ← 末尾の余分な空列（末尾カンマ）を無視
                dtype=str,
                chunksize=chunksize,
                na_filter=True,
                ...
                encoding="utf-8",
                skiprows=skiprows,           # 先頭 N 行をスキップ（メタ行除去）
                engine="python",             # 可変列/末尾カンマへの耐性を上げる
            ):
